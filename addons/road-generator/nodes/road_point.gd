...
func _set_prior_pt_init(value:NodePath):
	var _pre_assign: NodePath = prior_pt_init
	prior_pt_init = value
	if not is_instance_valid(container):
		return  # Might not be initialized yet.
...
func _set_next_pt_init(value:NodePath):
	var _pre_assign: NodePath = next_pt_init
	next_pt_init = value
	if not is_instance_valid(container):
		return  # Might not be initialized yet.
...
func _notification(what):
	...
	if what == NOTIFICATION_TRANSFORM_CHANGED:
		if _skip_next_on_transform:
			...
			return
		var low_poly: bool = Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT) and Engine.is_editor_hint()
...
func cross_container_connected() -> bool:
	if not is_on_edge():
		return false
	var _pr: RoadGraphNode = get_prior_road_node()
	if is_instance_valid(_pr) and _pr.container != self.container:
		return true
	var _nt: RoadGraphNode = get_next_road_node()
...
func get_prior_road_node(limit_same_container: bool = false) -> RoadGraphNode:
	...
	for _idx in range(len(container.edge_rp_locals)):
		...
		if not container.edge_containers[_idx]:
			return null
		var target_container: Node = container.get_node_or_null(container.edge_containers[_idx])
		if not is_instance_valid(target_container):
			push_warning("Invalid edge container on %s, refresh roads" % container.name)
			return null
		var reverse_rp: Node = target_container.get_node_or_null(container.edge_rp_targets[_idx])
...
func get_last_rp(direction: int) -> RoadPoint:
...
func assign_lanes():
	...
	var flips: Array[Variant] = [] # Track changes in direction between lanes.
	var only_fwd_rev: bool = true # If false, a non supported complex scenario.
	var fwd_rev: Array[Variant] = [LaneDir.FORWARD, LaneDir.REVERSE]
...
func get_fwd_lane_count() -> int:
	var td: Array[LaneDir] = traffic_dir
	var fwd_lane_count: int = 0
...
func update_traffic_dir(traffic_update):
	var fwd_lane_count: int = get_fwd_lane_count()
	var rev_lane_count: int = get_rev_lane_count()
	var lane_count: int = fwd_lane_count + rev_lane_count
...
func copy_settings_from(ref_road_point: RoadPoint, copy_transform: bool = true) -> void:
	var tmp_auto_lane: bool = ref_road_point.auto_lanes
	auto_lanes = false
...
func add_road_point(new_road_point: RoadPoint, direction):
	...
	var basis_z: Vector3 = new_road_point.transform.basis.z
