	...
	for _seg in mesh_parents:
		...
		if _seg is RoadIntersection:
			...
			if inter.container.flatten_terrain and inter.flatten_terrain:
				...
				# weren't scheduled
				var adj_segs: Array = intersection_adjacent_segments(inter)
				segs += adj_segs
...
func flatten_terrain_via_roadsegment_raycast(segment: RoadSegment) -> void:
	...
	var bounding_box_offset: Vector2 = Vector2(segment.start_point.global_position.x, segment.start_point.global_position.z)
	...
	for _m in missed:
		...
		var neighbour: Vector2 = Vector2(_m.x+neighbour_range,_m.y)
	...
	while x <= max.x:
		...
		while z <= max.z:
			...
			if closest_distance == 0.0:
				var _offset: Vector3 = world_pos - segment.start_point.global_position 
			...
			if closest_distance == flattened_curve.get_baked_length():
				var _offset: Vector3 = world_pos - segment.end_point.global_position # check this, also if can be flipped..???
...
func curve_3d_to_2d(curve: Curve3D) -> Curve2D:
	...
	for i in point_count:
		var pos3: Vector3 = curve.get_point_position(i)
		var in3: Vector3 = curve.get_point_in(i)
		var out3: Vector3 = curve.get_point_out(i)
		var tilt: float = curve.get_point_tilt(i)

		# Project to XZ plane (drop Y)
		var pos2 :Vector2 = Vector2(pos3.x, pos3.z)
		var in2 : Vector2= Vector2(in3.x, in3.z)
		var out2: Vector2 = Vector2(out3.x, out3.z)
...
func curve_2d_to_boundingbox(curve: Curve2D, start_width: float, end_width: float, offset: Vector2) -> PackedVector2Array:
	...
	# first tangent
	var extrapolated_neg_1: Vector2 = baked[0] - baked[1]
	var tangent: Vector2 = (baked[0] - extrapolated_neg_1).normalized()
	var perpendicular_right: Vector2 = tangent.rotated(deg_to_rad(90))
	var perpendicular_left: Vector2 = tangent.rotated(deg_to_rad(-90))
	...
	var extrapolated_last: Vector2 = baked[count - 1] + (baked[count - 1] - baked[count - 2])
